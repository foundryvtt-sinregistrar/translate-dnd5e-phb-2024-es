 ...
 */
(async () => {
    const PACK_ID = "dnd-players-handbook.feats";
    const OUTPUT_FILENAME = "dnd-players-handbook.feats-en.json";
    const LABEL = "Feats";
    ...
    try {
        ...
        // -----------------------------------------------------
        // Ordenación ASC con prioridad:
        //   1) claves que empiezan por número
        //   2) claves que empiezan por letra
        //   3) resto
        // Dentro de cada grupo: orden alfabético ASC
        // -----------------------------------------------------
        const getSortGroup = (key) => {
            const s = String(key || "");
        ...
        const compareIdsAscNumbersFirst = (a, b) => {
        ...
        const sortObjectByIdAscNumbersFirst = (obj) =>
            Object.fromEntries(
                Object.entries(obj || {}).sort(([a], [b]) => compareIdsAscNumbersFirst(a, b))
            );
        ...
        const serializeActivities = (item) => {
            ...
            return sortObjectByIdAscNumbersFirst(out);
        };
        ...
        const reorderEntryForFinalOutput = (entry) => ({
            ...
            folder: entry?.folder ?? "",
            activities: sortObjectByIdAscNumbersFirst(entry?.activities || {}),
            effects: sortObjectByIdAscNumbersFirst(entry?.effects || {}),
            advancement: sortObjectByIdAscNumbersFirst(entry?.advancement || {})
        });
        ...
        const orderedEntries = Object.fromEntries(
            Object.entries(sortObjectByIdAscNumbersFirst(entries)).map(([id, entry]) => [
                id,
        ...
        const output = {
            ...
            folders: sortObjectByKeyAsc(folders),
