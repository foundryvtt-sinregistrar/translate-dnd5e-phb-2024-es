 ...
 *
 * Estructura que genera y ordenación aplicada
 *
 * Estructura exportada:
 * {
 *   label,
 *   mapping: {
 *     activities: { path, converter },
 *     effects: { path, converter },
 *     advancement: { path, converter }
 *   },
 *   folders: {
 ...
 *
 * Notas:
 * - folders se exporta como objeto plano: { "Folder": "Folder" }
 * - folder se conserva como dato dentro de cada entry
 * - activities exporta solo:
 *   - name
 *   - target
 *   - activation.condition
 *   - description.chatFlavor
 * - effects exporta solo:
 *   - name
 *   - description
 * - advancement exporta solo:
 *   - title
 *   - hint
 * - No se exportan range ni duration en activities
 */
(async () => {
    const PACK_ID = "dnd-players-handbook.spells";
    const OUTPUT_FILENAME = "dnd-players-handbook.spells-en.json";
    const LABEL = "Spells";
    ...
    try {
        ...
        const compareAlphaAscNumbersFirst = (a, b) => {
            ...
            const aIsDigit = startsWithDigit(aStr);
            const bIsDigit = startsWithDigit(bStr);
        ...
        const sortObjectByKeyAscNumbersFirst = (obj) =>
            Object.fromEntries(
        ...
        const inferActivityNameByShape = (act) => {
            ...
            const hasAttackLike =
                act?.attack ||
                act?.roll?.type === "attack" ||
                act?.actionType === "mwak" ||
                act?.actionType === "rwak" ||
                act?.actionType === "msak" ||
                act?.actionType === "rsak";

            if (hasAttackLike) return "Attack";
            ...
            if (hasSaveLike) return "Save";

            const hasDamageLike =
                (Array.isArray(act?.damage?.parts) && act.damage.parts.length > 0) ||
                (Array.isArray(act?.damage?.onSave) && act.damage.onSave.length > 0) ||
                act?.roll?.type === "damage";

            if (hasDamageLike) return "Damage";

            const hasHealingLike =
                act?.healing ||
                act?.roll?.type === "heal";

            if (hasHealingLike) return "Heal";
        ...
        const getActivityName = (act) => {
            ...
            return inferActivityNameByShape(act);
        };
        ...
        const getActivityTarget = (act) => {
            ...
            if (templateType && templateSize !== undefined && templateSize !== null && templateSize !== "") {
                return `${templateSize} ${templateType}`;
            }
        ...
        const serializeActivities = (item) => {
            ...
            return sortObjectByKeyAscNumbersFirst(out);
        };
        ...
        const reorderEntryForFinalOutput = (entry) => ({
            ...
            folder: entry?.folder ?? "",
            activities: sortObjectByKeyAscNumbersFirst(entry?.activities || {}),
            effects: sortObjectByKeyAscNumbersFirst(entry?.effects || {}),
            advancement: sortObjectByKeyAscNumbersFirst(entry?.advancement || {})
        });
        ...
        const orderedEntries = Object.fromEntries(
            Object.entries(sortObjectByKeyAscNumbersFirst(entries)).map(([id, entry]) => [
                id,
